...
function FormationDetails() {
  const { id } = useParams(); // Extract formation ID from URL
  ...
  const fetchFormation = async () => {
    ...
    try {
      ...
      initializeFormationSlots(response.data);
    } catch (error) {
      console.error('Error fetching formation:', error);
      alert('Failed to fetch formation details.');
      navigate('/formations'); // Redirect back to list if error occurs
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      fetchFormation(); // Refresh formation data
    } catch (error) {
  ...
  const handleDelete = async () => {
    ...
    if (window.confirm('Are you sure you want to delete this formation?')) {
      try {
        ...
        navigate('/formations'); // Redirect to formations list
      } catch (error) {
  ...
  return (
    ...
    >
      ...
      {formation ? (
        ...
        <Typography>Loading formation details...</Typography>
      )}
    </Paper>
  );
}
